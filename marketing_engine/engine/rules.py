...
com três peças:
  ...
  1. finder(db)        -> lista de clients elegíveis para a regra
  2. flow               -> "auto"     dispara direto via WhatsApp
                           "approval" cai na fila marketing_approval_queue
  3. build_message(c)   -> "auto":     retorna a string final da mensagem
                           "approval": retorna um dict {strategy, suggested_message, context}

Campos reais de clients usados aqui:
  client["name"]       -> nome do cliente
  client["phone"]      -> telefone
  client["birthdate"]  -> data de nascimento (YYYY-MM-DD)
  client["last_visit"] -> última visita (date)

Nos tools de aprovação, usa-se client_id (integer) em vez de patient_id.
...
def find_birthday_clients(db) -> list[dict]:
    return db_module.get_clients_with_birthday_today(db)


def build_birthday_message(client: dict) -> str:
...
def find_nps_targets(db) -> list[dict]:
    """TODO: clients com atendimento concluído há ~24h e sem NPS respondido."""
    return []


def build_nps_message(client: dict) -> str:
    first_name = client["name"].split(" ")[0]
...
def find_escassez_targets(db) -> list[dict]:
    """TODO: clients com agenda aberta e poucas vagas restantes na semana."""
    return []


def build_escassez_message(client: dict) -> str:
    first_name = client["name"].split(" ")[0]
...
def build_appointment_reminder_message(client: dict) -> str:
    first_name = client["name"].split(" ")[0]
...
def find_confirmation_targets(db) -> list[dict]:
    """TODO: agendamentos recém-criados aguardando confirmação do cliente."""
    return []


def build_confirmation_message(client: dict) -> str:
    first_name = client["name"].split(" ")[0]
...
def build_post_procedure_message(client: dict) -> str:
    first_name = client["name"].split(" ")[0]
...
def find_package_ending_targets(db) -> list[dict]:
    """TODO: clients com 1-2 sessões restantes em pacotes contratados."""
    return []


def build_package_ending_message(client: dict) -> str:
    first_name = client["name"].split(" ")[0]
...
def find_referral_targets(db) -> list[dict]:
    """TODO: clients com NPS alto (promotores) nos últimos 30 dias."""
    return []


def build_referral_message(client: dict) -> str:
    first_name = client["name"].split(" ")[0]
...
def build_reaquecimento_strategy(client: dict) -> dict:
    """
    ...
    return {
        ...
        "context": {
            ...
            "reason": "Lead frio — sem visita recente (60+ dias)",
        },
    }
...
def find_upsell_targets(db) -> list[dict]:
    """TODO: clients com histórico de tratamento X, candidatos a tratamento complementar Y."""
    return []


def build_upsell_strategy(client: dict) -> dict:
    first_name = client["name"].split(" ")[0]
    return {
        "strategy": "upsell",
...
def find_no_show_targets(db) -> list[dict]:
    """
    Busca appointments com status 'no_show' no último dia.
    O JOIN retorna clients aninhado em cada row — extraímos o dict do client.
    """
    raw = db_module.get_no_show_appointments(db, days_window=1)
    return [row["clients"] for row in raw if row.get("clients")]


def build_no_show_strategy(client: dict) -> dict:
    first_name = client["name"].split(" ")[0]
    return {
        "strategy": "no_show",
        "suggested_message": f"Oi {first_name}, sentimos sua falta hoje! Quer remarcar?",
        "context": {"reason": "Faltou à consulta no último dia"},
    }
...
def find_reactivation_targets(db) -> list[dict]:
    """TODO: clients inativos há 6+ meses (diferente de reaquecimento, que é 60 dias)."""
    return []


def build_reactivation_strategy(client: dict) -> dict:
    first_name = client["name"].split(" ")[0]
    return {
...
def build_open_budget_strategy(client: dict) -> dict:
    first_name = client["name"].split(" ")[0]
    return {
...
def find_post_complaint_targets(db) -> list[dict]:
    """TODO: clients que abriram reclamação/NPS detrator nos últimos dias."""
    return []


def build_post_complaint_strategy(client: dict) -> dict:
    first_name = client["name"].split(" ")[0]
    return {
...
TOOL_REGISTRY: dict[str, Tool] = {
    # ---- Fluxo Automático (8) ----
    "aniversario": Tool(
        "aniversario", "auto", find_birthday_clients, build_birthday_message,
    ...
    "indicacao_referral": Tool(
        ...
        "Convite para indicação, enviado a clients promotores (NPS alto).",
    ),
    ...
    "no_show": Tool(
        ...
        "Recuperação de client que faltou à consulta — exige revisão (tom sensível).",
    ),
    "reativacao_inativos": Tool(
        "reativacao_inativos", "approval", find_reactivation_targets, build_reactivation_strategy,
        "Reativação de clients inativos há 6+ meses — exige revisão.",
    ),
