...
const FERRAMENTAS = [
  ...
  { id: 20, nome: '2º Lembrete de Manutenção (Geladeira)', grupo: 'B', gatilho: 'Hoje = data da consulta + servicos.dias_para_retorno + 15 dias de carência (Etapa 4).', le: ['servicos', 'appointments'], escreve: 'pending', janela: '45 dias', tags: ['nome_paciente', 'nome_servico', 'dias_retorno'], obs: 'Dispara o 2º lembrete e marca a entrada do serviço na geladeira (espera) para clientes que não retornaram.', passos: ['1. Busca consultas finalizadas cuja data bate com hoje − (dias_para_retorno + 15).', '2. Confirma que a cliente NÃO agendou nem realizou o procedimento novamente.', '3. Gera a mensagem como pending para revisão humana.', '4. A recepcionista valida antes do envio.'] },
  { id: 21, nome: 'Resgate Inteligente (Oferta Personalizada)', grupo: 'B', gatilho: 'Hoje = data da consulta + servicos.dias_para_retorno + 15 dias + 30 dias de geladeira (Etapa 5).', le: ['servicos', 'appointments'], escreve: 'pending', janela: '75 dias', tags: ['nome_paciente', 'nome_servico', 'outro_servico', 'dias_retorno'], obs: 'Resgate com oferta especial após 30 dias na geladeira, sugerindo outro procedimento que a cliente já realizou.', passos: ['1. Busca consultas finalizadas cuja data bate com hoje − (dias_para_retorno + 45).', '2. Confirma que a cliente ainda não retornou para este serviço.', '3. Busca outro serviço feito pela cliente para personalizar a oferta.', '4. Gera a mensagem com oferta e condição especial como pending.'] },
];
...
const TABELAS = [
  {
    nome: 'message_templates', papel: 'O "texto" das ferramentas de marketing',
    desc: 'Uma linha por ferramenta (tool_id 1–21). O motor NUNCA tem texto fixo: no momento do gatilho ele faz SELECT aqui e renderiza as tags {{nome_paciente}}, {{hora_consulta}}, etc. Editar aqui (aba Motor → Ferramentas) vale na hora, sem reiniciar nenhum cronômetro.',
