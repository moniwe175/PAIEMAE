...
function MiniKanban({ krId, tasks, onMoveTask, krStatus }) {
  const { currentDay, addTask, updateTask, deleteTask } = useOKR();
...
function KeyResultBlock({ objId, kr, expanded, onToggle, onMoveTask }) {
  const { currentDay, totalDays, updateKeyResult, deleteKeyResult } = useOKR();
  const progress = getKRProgress(kr.tasks);
  const expectedProgress = getExpectedProgress(currentDay, totalDays);
...
function ObjectiveBlock({ objective, onMoveTask, expandedKRs, toggleKR }) {
  const { currentDay, totalDays, updateObjective, deleteObjective, addKeyResult } = useOKR();
...
function InsightsPanel({ allTasks, allKRs, expectedProgress }) {
  const { currentDay, totalDays } = useOKR();
...
export default function Estrategia() {
  const { okrData, moveTask, addObjective, currentDay, totalDays, currentMonth } = useOKR();
    ...
    <div>
      ...
      <div className="page-header">
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <h1 className="page-title" style={{ marginBottom: 2 }}>{currentMonth}</h1>
