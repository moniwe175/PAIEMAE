...
export async function fetchStickyNotes() {
  try {
    ...
    if (error) {
      console.warn('[okrService] Note: sticky_notes table not found or unavailable:', error.message);
      return { data: [], error };
    }
    return { data: data || [], error: null };
  } catch (err) {
    console.warn('[okrService] Exception fetching sticky_notes:', err?.message || err);
...
export async function insertStickyNote(note) {
  try {
    const { data, error } = await supabase
      ...
      .select()
      .single();
    if (error) console.warn('[okrService] Error inserting sticky_note:', error.message);
    return { data, error };
  } catch (err) {
...
export async function updateStickyNote(id, updates) {
  try {
    const { data, error } = await supabase
      ...
      .select()
      .single();
    if (error) console.warn('[okrService] Error updating sticky_note:', error.message);
    return { data, error };
  } catch (err) {
    return { data: null, error: err };
  }
...
export async function deleteStickyNote(id) {
  try {
    const { error } = await supabase
      ...
      .delete()
      .eq('id', id);
    if (error) console.warn('[okrService] Error deleting sticky_note:', error.message);
    return { error };
  } catch (err) {
    return { error: err };
  }
